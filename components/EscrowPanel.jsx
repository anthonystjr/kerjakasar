...
export default function EscrowPanel({ escrow: escrowProp, applicationId, jobBudget = 0, isClient, onUpdate }) {
  // localEscrow memungkinkan update UI langsung tanpa tunggu parent re-render
  const [localEscrow, setLocalEscrow] = useState(escrowProp)
  ...
  const call = async (url, body, method = 'POST') => {
    ...
    try {
      ...
      if (!r.ok) { setErr(d.error || 'Terjadi kesalahan'); return null }
      if (d.escrow) setLocalEscrow(d.escrow) // ← update lokal langsung
      setShowForm(null)
      onUpdate?.()
      return d.escrow
  ...
  if (!escrow) {
    if (!isClient) return (
      <div style={s.box}><p style={s.info}>Escrow belum dibuat oleh client.</p></div>
    )
    return (
      <div style={s.box}>
  ...
  // ── Amount belum diisi (< 1000) ───────────────────────────────────
  if (isClient && st === 'pending_payment' && Number(escrow.amount) < 1000) {
    return (
      <div style={s.box}>
        <h3 style={s.title}>🔒 Isi Jumlah Dana</h3>
        <p style={s.info}>Masukkan jumlah dana yang akan dikunci untuk proyek ini.</p>
        <div style={s.form}>
          <label style={s.label}>Jumlah Dana (Rp)</label>
          <input type="number" value={amount} min={1000} onChange={e => setAmount(e.target.value)}
            style={s.input} placeholder="cth: 500000" />
  ...
  // ── Ada escrow, tampilkan status + aksi ──────────────────────────
  return (
    <div style={s.box}>
      ...
      {/* Client: pending_payment → konfirmasi transfer */}
      {isClient && st === 'pending_payment' && (
        showForm === 'fund' ? (
          <div style={s.form}>
            <p style={s.info}>Masukkan referensi transfer (no. bukti / nama bank).</p>
            <label style={s.label}>Referensi Pembayaran</label>
            <input value={paymentRef} onChange={e => setPaymentRef(e.target.value)}
              style={s.input} placeholder="cth: BCA-20260607-XXXXX" />
            <div style={s.row}>
              <button style={s.btnSec} onClick={() => setShowForm(null)}>Batal</button>
              <button style={s.btnPri} disabled={loading || !paymentRef}
      ...
      {/* Client: funded → cairkan */}
      {isClient && st === 'funded' && (
        showForm === 'release' ? (
          <div style={s.form}>
            <p style={s.info}>Pekerjaan selesai? Dana akan dicairkan ke talent.</p>
      ...
      {!isClient && st === 'funded' && (
        <div style={{ ...s.badge, ...color, display: 'block', textAlign: 'center', padding: '10px 0' }}>
          Dana sudah dikunci. Kerjakan proyek dan client akan mencairkan setelah selesai.
        </div>
      )}

      {/* Talent: released */}
      {!isClient && st === 'released' && (
        <p style={{ ...s.info, color: '#166534' }}>✅ Dana telah dicairkan. Cek rekening / dompetmu.</p>
      )}
    </div>
  )
}
