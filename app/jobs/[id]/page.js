        ...
        <div style={s.card}>
          <div style={s.headerRow}>
            <div style={{ flex: 1 }}>
              ...
              <div style={s.metaRow}>
                <span style={s.meta}>📍 {job.lokasi}</span>
                <span style={s.meta}>💰 Rp {Number(job.budget_min).toLocaleString('id-ID')} – {Number(job.budget_max).toLocaleString('id-ID')}</span>
                <span style={s.meta}>📅 {new Date(job.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div style={s.card}>
                ...
                <div style={s.posterRow}>
                  <div style={s.posterAvatar}>
                    {poster.foto
                      ? <img src={poster.foto} alt={poster.nama} style={s.posterAvatarImg} />
            ...
            <div style={s.card}>
              {!session ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {poster && (
                    <button
                      onClick={() => router.push(`/chat?jobId=${job.id}&with=${poster.id}`)}
                      style={s.btnChat}
                    >
                      💬 Chat dengan Poster
                    </button>
